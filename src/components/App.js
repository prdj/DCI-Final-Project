...
const App = () => {
  ...
  const onKeyDown = (e) => {
    ...
    console.log(`I AM UPDATING THE OSC WITH`, { value });
  ...
  useEffect(() => {
    document.addEventListener('mousedown', (e) => {
      ...
      function playNote() {
        let IndexClick = arrayNotes.findIndex(
          (x) => x.note === click
        );
        setKeyPressed(IndexClick.toString());
        // console.log(
        //   `You pressed: ${IndexClick.toString()}`
        // );
      }
  ...
  useEffect(() => {
    // console.log({ sound });
    // console.log(`VALUE APPJS 2`, { value });
    // EVERY TIME VALUE OR SOUND CHANGES I WILL REMOVE AND ADD THE EVENTLISTENER SO IT CAN TAKE THE NEW SOUND OR VALUE
